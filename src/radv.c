...
void icmp6_packet(time_t now)
{
  ...
  if (packet[0] == ICMP6_ECHO_REPLY)
	...
	my_syslog(MS_DHCP | LOG_INFO, "RTR-SOLICIT(%s) %s", interface, mac);
      ...
      /* If the incoming interface is also an alias of some other one
         (as specified by the --bridge-interfaces option), maybe also
         send an RA for the aliased interface. */
	  ...
	  if (!tmp)
            {
              ...
              for (aparam.bridge = daemon->bridges;
                ...
                if (if_nametoindex(aparam.bridge->iface) == param.iface)
                  {
